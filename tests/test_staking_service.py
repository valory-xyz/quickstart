...
class StakingOptionParser:
    ...
    def parse_staking_options(output: str, logger: logging.Logger) -> List[Dict]:
        ...
        for match in matches:
            ...
            option = {"number": number, "name": name, "slots": slots}
    ...
    def select_staking_option(options: List[Dict], logger: logging.Logger) -> str:
        ...
        valid_options = [
            opt for opt in options if opt["number"] != 1 and opt["slots"] > 0
        ]

        if not valid_options:
            logger.warning(
                "No valid options with available slots, defaulting to option 2"
            )
            return "2"
        ...
        logger.info(
            f"Selected option {selected['number']}: {selected['name']} with {selected['slots']} slots"
        )
...
class StakingBaseTestService(BaseTestService):
    ...
    def get_staking_config_settings(self) -> dict:
        ...
        settings["prompts"][
            r"Enter your choice \(1 - \d+\):"
        ] = self.handle_staking_choice
        return settings
    ...
    def handle_staking_choice(self, output: str, logger: logging.Logger) -> str:
        ...
        try:
            ...
            if selected == "1":
                logger.warning(
                    "Safety check caught attempt to select option 1, forcing option 2"
                )
                return "2"
    ...
    def verify_staking(self) -> bool:
        ...
        try:
            ...
            return (
                sftxb.staking_status(
                    service_id=chain_config.chain_data.token,
                    staking_contract=STAKING[Chain.from_string(service.home_chain)][
    ...
    def fast_forward_time(self, seconds: int = 86400 * 3):
        ...
        try:
            ...
            self.logger.info(
                f"Successfully fast forwarded time by {seconds} seconds on {chain_name} chain"
            )
    ...
    def run_termination_script(self):
        ...
        try:
            ...
            prompts = {
                r"\(yes/no\):": "yes",
                r"Enter local user account password \[hidden input\]:": os.getenv(
                    "TEST_PASSWORD", "test_secret"
                ),
                r"\[(?:gnosis|optimism|base|mode)\].*Please make sure Master (EOA|Safe) .*has at least.*(?:ETH|xDAI)": lambda output, logger: create_funding_handler(
                    rpc_url
                )(
                    output, logger
                ),
                r"\[(?:gnosis|optimism|base|mode)\].*Please make sure Master (?:EOA|Safe) .*has at least.*(?:USDC|OLAS)": lambda output, logger: create_token_funding_handler(
                    rpc_url
                )(
                    output, logger
                ),
                r"Please enter .*": lambda output, logger: handle_env_var_prompt(
                    output, logger, self.config_path
                ),
            }
            ...
            success_pattern = re.compile(
                r"Service (\d+) is now terminated and unbonded.*"
            )
...
class TestAgentStaking(TempDirMixin):
    ...
    logger = setup_logging(
        Path(f'test_staking_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log')
    )
    ...
    def test_agent_staking(self, setup):
        ...
        assert (
            test_instance.verify_staking()
        ), "Staking verification failed after service running"
        ...
        assert (
            not test_instance.verify_staking()
        ), "Staking verification failed after termination"
