...
def test_parse_optimus_config_happy_path(
...
def test_parse_optimus_config_missing_required_fields(
    ...
    optimus_path.mkdir(parents=True)
    (optimus_path / "local_config.json").write_text(
        json.dumps({"use_staking": False}), encoding="utf-8"
    )
    monkeypatch.setattr(migrate, "OPTIMUS_PATH", optimus_path)
    monkeypatch.setattr(migrate, "print_section", lambda *_args, **_kwargs: None)
...
def test_copy_optimus_to_operate(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
...
def test_main_returns_when_optimus_missing(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
...
def test_create_operate_config_skips_missing_and_non_matching_service_dirs(
    ...
    (non_match / "config.json").write_text(
        json.dumps({"name": "other/service"}), encoding="utf-8"
    )
...
def test_main_success_calls_all_steps(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    ...
    monkeypatch.setattr(
        migrate, "parse_optimus_config", lambda: calls.append("parse") or optimus_config
    )
    monkeypatch.setattr(
        migrate, "copy_optimus_to_operate", lambda: calls.append("copy")
    )
    monkeypatch.setattr(
        migrate,
        "create_operate_config",
        lambda cfg, name: calls.append(f"create:{name}:{cfg.principal_chain}"),
    )
    monkeypatch.setattr(
        migrate, "print_section", lambda msg: calls.append(f"section:{msg}")
    )
