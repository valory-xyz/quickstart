...
def test_populate_ipfs_contents_falls_back_to_non_metadata_url(
...
def test_query_mech_events_subgraph_paginates(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    class _FakeClient:
        def execute(
            self, _query: Any, variable_values: dict[str, Any]
        ) -> dict[str, Any]:
    ...
    monkeypatch.setattr(
        mech_events, "RequestsHTTPTransport", lambda *_args, **_kwargs: object()
    )
...
def test_update_mech_events_db_updates_missing_and_incomplete_events(
    ...
    monkeypatch.setattr(
        mech_events.MechBaseEvent, "_populate_ipfs_contents", lambda self: None
    )
...
def test_get_mech_events_updates_then_returns_sender_events(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_get_mech_requests_filters_by_timestamp(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_populate_ipfs_contents_warns_when_no_hash(
...
def test_populate_ipfs_contents_uses_ipfs_hash_bytes(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_populate_ipfs_contents_handles_generic_exception(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_update_mech_events_db_handles_keyboard_interrupt(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_update_mech_events_db_handles_generic_exception(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
