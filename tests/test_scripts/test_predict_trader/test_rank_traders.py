...
def _stats_row(
    roi: float, trades: int
) -> dict[rank_traders.MarketAttribute, dict[rank_traders.MarketState, Any]]:
...
def test_print_user_summary_sorts_descending(
...
def test_print_progress_bar_writes_expected_output(
    ...
    rank_traders._print_progress_bar(
        iteration=5, total=10, prefix="P", suffix="S", length=10, fill="#"
    )
...
def test_main_execution_path(
    ...
    monkeypatch.setattr(
        trades_module,
        "parse_user",
        lambda _rpc, _creator, _trades_json, _stats: (
            "ignored",
            _stats_row(roi=0.25, trades=1),
        ),
    )
