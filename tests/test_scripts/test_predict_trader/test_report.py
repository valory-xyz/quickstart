...
def test_color_helpers(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert report._color_string("ok", report.ColorCode.GREEN).startswith(
...
def test_trades_since_message(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert (
        report._trades_since_message(trades_json, utc_ts=10) == "2 trades on 1 markets"
    )
...
def test_calculate_retrades_since(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    filtered, n_unique, n_trades, n_retrades = report._calculate_retrades_since(
        trades_json, utc_ts=10
    )
...
def test_calculate_retrades_since_raises_on_missing_market_id(
...
def test_retrade_average_and_max_messages(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert (
        report._retrades_since_message(2, 3, 1)
        == "1 re-trades on total 3 trades in 2 markets"
    )
    ...
    assert (
        report._max_trades_per_market_since_message({"m1": 3, "m2": 1})
        == "3 trades per market"
    )
...
def test_print_helpers_emit_expected_text(
...
def test_get_agent_status_container_running(
...
def _run_report_main(
    ...
    *,
    ...
    monkeypatch.setattr(
        utils_module, "get_service_from_config", lambda *_args, **_kwargs: service
    )
    monkeypatch.setattr(
        run_service,
        "load_local_config",
    ...
    monkeypatch.setattr(
        trades_module, "get_mech_requests", lambda *_args, **_kwargs: {}
    )
    monkeypatch.setattr(
        trades_module, "get_mech_statistics", lambda *_args, **_kwargs: {}
    )
    monkeypatch.setattr(
        trades_module,
        "_query_omen_xdai_subgraph",
        lambda *_args, **_kwargs: trades_json,
    )
    monkeypatch.setattr(
        trades_module, "parse_user", lambda *_args, **_kwargs: ("ok", stats_table)
    )
    ...
    monkeypatch.setattr(
        trades_module, "get_token_balance", lambda *_args, **_kwargs: 2 * 10**18
    )
    monkeypatch.setattr(
        profiles, "get_staking_contract", lambda **_kwargs: staking_token_address
    )
...
def test_report_main_exits_without_wallet(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
...
def test_report_main_exits_without_gnosis_safe(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
...
def test_report_main_runs_non_staked_flow(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
...
def test_report_main_runs_staked_flow_with_fallbacks(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
...
def test_report_main_evicted_and_staking_try_exception(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
