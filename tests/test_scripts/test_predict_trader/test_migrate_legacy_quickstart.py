...
def test_decrypt_private_keys_without_password(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
...
def test_decrypt_private_keys_with_password(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
...
def test_parse_trader_runner_reads_files_and_retries_password(
	...
	(trader_runner / "service_safe_address.txt").write_text("0x" + "c" * 40, encoding="utf-8")
	(trader_runner / ".env").write_text("SUBGRAPH_API_KEY=from-file\n", encoding="utf-8")
	...
	monkeypatch.setattr(migrate, "getpass", lambda *_args, **_kwargs: next(password_inputs))
...
def test_main_exits_when_trader_runner_missing(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
...
def test_populate_operate_full_flow(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
	...
	monkeypatch.setattr(migrate.Account, "encrypt", lambda private_key, password: {"enc": private_key, "pwd": password})

	root = tmp_path / "repo"
	(root / ".trader_runner").mkdir(parents=True)
	(root / "data").mkdir(parents=True)
	(root / ".trader_runner" / "mech_events.json").write_text('{"k":1}', encoding="utf-8")
	...
	def _fake_decrypt(path: Path, _password: str):
		if "operator" in path.name:
			return {"address": "0xmaster", "private_key": "0xpkmaster", "ledger": "ethereum"}
...
def test_populate_operate_existing_service_and_copy_failure(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
	...
	(root / ".trader_runner").mkdir(parents=True)
	(root / "data").mkdir(parents=True)
	(root / ".trader_runner" / "mech_events.json").write_text("not-json", encoding="utf-8")
	...
	monkeypatch.setattr(migrate, "decrypt_private_keys", lambda *_a, **_k: {"address": "0xagent", "private_key": "0xpk", "ledger": "ethereum"})
	...
	service = SimpleNamespace(path=tmp_path / "service", agent_addresses=[], chain_configs={migrate.Chain.GNOSIS.value: SimpleNamespace(chain_data=SimpleNamespace(token=0, multisig="0x"))}, store=lambda: None)
	service.path.mkdir()
	...
	service_template = {"name": "svc", "configurations": {migrate.Chain.GNOSIS.value: {}}}
	trader_data = migrate.TraderData(
		password="pwd",
		agent_eoa=tmp_path / "agent.txt",
		master_eoa=tmp_path / "operator.txt",
...
def test_populate_operate_copy_mech_events_failure(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
	...
	operate_home.mkdir()
	monkeypatch.setattr(migrate, "OPERATE_HOME", operate_home)
	monkeypatch.setattr(migrate, "QuickstartConfig", lambda **_kwargs: SimpleNamespace(store=lambda: None))
	keys_path = tmp_path / "keysmanager"
	keys_path.mkdir()
	monkeypatch.setattr(migrate, "decrypt_private_keys", lambda *_a, **_k: {"address": "0xagent", "private_key": "0xpk", "ledger": "ethereum"})

	master_wallet = SimpleNamespace(safes={migrate.Chain.GNOSIS: "0xsafe"})
	wallet_manager = SimpleNamespace(exists=lambda _lt: True, setup=lambda: None, load=lambda _lt: master_wallet, path=tmp_path / "wallets")
	(wallet_manager.path).mkdir()
	service = SimpleNamespace(path=tmp_path / "service", agent_addresses=[], chain_configs={migrate.Chain.GNOSIS.value: SimpleNamespace(chain_data=SimpleNamespace(token=1, multisig="0x"))}, store=lambda: None)
	service.path.mkdir()
	operate = SimpleNamespace(user_account=object(), password="", wallet_manager=wallet_manager, keys_manager=SimpleNamespace(path=keys_path), setup=lambda: None, create_user_account=lambda _p: None, service_manager=lambda: SimpleNamespace(json=[]))
	monkeypatch.setattr(migrate, "get_service", lambda *_a, **_k: service)
...
def test_populate_operate_existing_service_overwrite_true(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
	...
	service.path.mkdir()
	service_manager = SimpleNamespace(json=[{"name": "svc"}])
	operate = SimpleNamespace(user_account=object(), password="", wallet_manager=wallet_manager, keys_manager=SimpleNamespace(path=keys_path), setup=lambda: None, create_user_account=lambda _p: None, service_manager=lambda: service_manager)
	monkeypatch.setattr(migrate, "get_service", lambda *_a, **_k: service)
...
def _build_migrate_context(tmp_path: Path, *, staking_status, service_owner, on_chain_states, xdai_balance, tx_value_after_gas):
	"""Build fake operate/trader/service context for migrate_to_master_safe tests."""

	class _EthAPI:
		def __init__(self):
			self.account = SimpleNamespace(sign_transaction=lambda tx, private_key: SimpleNamespace(rawTransaction=b"raw"))
	...
	class _MasterWallet:
		def __init__(self):
			...
			self.crypto = SimpleNamespace(address="0xmaster", sign_transaction=lambda transaction: "signed")

		def ledger_api(self, *_args, **_kwargs):
			class _L:
				def __init__(self):
					self.api = SimpleNamespace(eth=SimpleNamespace(wait_for_transaction_receipt=lambda _tx: None))

				def get_transfer_transaction(self, **_kwargs):
					return {"value": xdai_balance, "gas": 1, "maxFeePerGas": xdai_balance - tx_value_after_gas}
	...
	service = SimpleNamespace(
		...
		chain_configs={migrate.Chain.GNOSIS.value: SimpleNamespace(chain_data=SimpleNamespace(token=99), ledger_config=SimpleNamespace(rpc="http://rpc"))},
	)
...
def test_migrate_to_master_safe_paths(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
	...
	operate, trader_data, service, ocm = _build_migrate_context(
		...
		on_chain_states=[migrate.OnChainState.ACTIVE_REGISTRATION, migrate.OnChainState.TERMINATED_BONDED],
		xdai_balance=100,
	...
	service_registry = SimpleNamespace(
		...
		get_instance=lambda **_kwargs: SimpleNamespace(
			functions=SimpleNamespace(
				transferFrom=lambda *_args: SimpleNamespace(build_transaction=lambda _tx: {"tx": 1})
			)
		),
	)
	erc20 = SimpleNamespace(
		get_instance=lambda **_kwargs: SimpleNamespace(
			functions=SimpleNamespace(
				transfer=lambda *_args: SimpleNamespace(build_transaction=lambda _tx: {"tx": 2})
			)
		),
	)
	monkeypatch.setattr(migrate, "registry_contracts", SimpleNamespace(service_registry=service_registry, erc20=erc20))
	monkeypatch.setattr(migrate, "get_assets_balances", lambda **_kwargs: {"0xmaster": {"0xasset": 5, "0xzero": 0}})
	monkeypatch.setattr(migrate, "get_asset_balance", lambda **_kwargs: 100)
	monkeypatch.setattr(migrate, "CHAIN_TO_METADATA", {migrate.Chain.GNOSIS.value: {"gasFundReq": 1}})
...
def test_migrate_to_master_safe_cancelled_and_short_duration(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
...
def test_migrate_to_master_safe_owner_mismatch_and_early_returns(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
	...
	monkeypatch.setattr(migrate, "registry_contracts", SimpleNamespace(service_registry=SimpleNamespace(get_service_owner=lambda **_kwargs: {"service_owner": "0xother"}), erc20=SimpleNamespace(get_instance=lambda **_kwargs: None)))
	monkeypatch.setattr(migrate, "get_assets_balances", lambda **_kwargs: {"0xmaster": {}})
	monkeypatch.setattr(migrate, "get_asset_balance", lambda **_kwargs: 0)
	monkeypatch.setattr(migrate, "CHAIN_TO_METADATA", {migrate.Chain.GNOSIS.value: {"gasFundReq": 100}})
	monkeypatch.setattr(migrate, "ERC20_TOKENS", [{migrate.Chain.GNOSIS: "0xasset"}])
...
def test_migrate_to_master_safe_transferable_amount_non_positive(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
	...
	monkeypatch.setattr(migrate, "registry_contracts", SimpleNamespace(service_registry=SimpleNamespace(get_service_owner=lambda **_kwargs: {"service_owner": "0xsafe"}), erc20=SimpleNamespace(get_instance=lambda **_kwargs: None)))
	monkeypatch.setattr(migrate, "get_assets_balances", lambda **_kwargs: {"0xmaster": {}})
	monkeypatch.setattr(migrate, "get_asset_balance", lambda **_kwargs: 1)
	monkeypatch.setattr(migrate, "CHAIN_TO_METADATA", {migrate.Chain.GNOSIS.value: {"gasFundReq": 10}})
	monkeypatch.setattr(migrate, "ERC20_TOKENS", [{migrate.Chain.GNOSIS: "0xasset"}])

	migrate.migrate_to_master_safe(operate, trader_data, service)


def test_migrate_to_master_safe_transfer_tx_value_non_positive(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
	...
	monkeypatch.setattr(migrate, "get_assets_balances", lambda **_kwargs: {"0xmaster": {}})
	monkeypatch.setattr(migrate, "get_asset_balance", lambda **_kwargs: 100)
	monkeypatch.setattr(migrate, "CHAIN_TO_METADATA", {migrate.Chain.GNOSIS.value: {"gasFundReq": 0}})
	monkeypatch.setattr(migrate, "ERC20_TOKENS", [{migrate.Chain.GNOSIS: "0xasset"}])

	migrate.migrate_to_master_safe(operate, trader_data, service)


def test_main_success_and_insecure_password_notice(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
	"""main should run migration flow and show insecure password notice when empty password."""

	monkeypatch.setattr(migrate, "TRADER_RUNNER_PATH", tmp_path / ".trader_runner")
	(tmp_path / ".trader_runner").mkdir()
	config_path = tmp_path / "config.json"
	config_path.write_text('{"name":"svc","configurations":{"gnosis":{}}}', encoding="utf-8")
...
def test_module_main_entrypoint(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
	...
	monkeypatch.setattr(sys, "argv", ["migrate_legacy_quickstart.py", str(tmp_path / "cfg.json")])

	# Execute via runpy so module-level argparse and main call are covered.
	with pytest.raises(SystemExit):
		runpy.run_module("scripts.predict_trader.migrate_legacy_quickstart", run_name="__main__")
