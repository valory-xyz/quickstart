...
def test_parse_args_without_creator_uses_config(
    ...
    (tmp_path / "configs" / "config_predict_trader.json").write_text(
        "{}", encoding="utf-8"
    )
    ...
    class _Cfg:
        chain_configs = {
            "gnosis": type(
                "_X",
                (),
                {"chain_data": type("_Y", (), {"multisig": "0x" + "b" * 40})()},
            )()
        }

    monkeypatch.setattr(trades, "SCRIPT_PATH", script_path)
    monkeypatch.setattr(
        trades, "get_service_from_config", lambda *_args, **_kwargs: _Cfg()
    )
...
def test_query_omen_xdai_subgraph_paginates(
    ...
    responses = [
        {
            "data": {
                "fpmmTrades": [
                    {"id": "1", "creationTimestamp": "10", "fpmm": {"id": "m1"}}
                ]
            }
        },
        {"data": {"fpmmTrades": []}},
    ...
    requests_mock.post(
        url, [{"json": responses[0]}, {"json": responses[1]}, {"json": responses[2]}]
    )
...
def test_compute_totals_recomputes_derived_fields() -> None:
    ...
    table = {
        row: {col: 0 for col in trades.STATS_TABLE_COLS}
        for row in trades.STATS_TABLE_ROWS
    }
...
def test_get_market_state_paths(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    open_market = {
        "currentAnswer": None,
        "openingTimestamp": str(1735790400),
    }  # 2025-01-02
    pending_market = {
        "currentAnswer": None,
        "openingTimestamp": str(1735603200),
    }  # 2024-12-31
    arbitrating_market = {
        "currentAnswer": "0x0",
    ...
    finalizing_market = {
        ...
        "answerFinalizedTimestamp": str(1735776000),
    }
    closed_market = {
        "currentAnswer": "0x0",
        "isPendingArbitration": False,
        "answerFinalizedTimestamp": str(1735689600),
    }
    ...
    assert (
        trades._get_market_state(arbitrating_market) == trades.MarketState.ARBITRATING
    )
...
def test_format_table_and_mech_statistics() -> None:
    ...
    mech_requests = {
        "a": {
            "ipfs_contents": {"tool": "foo", "prompt": 'Ask "What is this?"'},
            "fee": 10,
        },
        "b": {"ipfs_contents": {"tool": "foo", "prompt": "Simple prompt"}, "fee": 5},
        "c": {
            "ipfs_contents": {"tool": trades.IRRELEVANT_TOOLS[0], "prompt": "skip"},
            "fee": 99,
        },
    }
...
def test_balance_helpers_use_rpc(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert (
        trades.get_token_balance("0x" + "2" * 40, "0x" + "3" * 40, "http://rpc") == 11
    )
...
def test_parse_user_covers_status_branches(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        trades, "_get_market_state", lambda fpmm: status_map[fpmm["id"]]
    )
    monkeypatch.setattr(
        trades,
    ...
    monkeypatch.setattr(
        trades, "get_token_balance", lambda *_args, **_kwargs: 2 * 10**18
    )
    ...
    output, table = trades.parse_user(
        "http://rpc", "0x" + "a" * 40, trades_json, mech_stats
    )
    ...
    assert (
        table[trades.MarketAttribute.NUM_INVALID_MARKET][trades.MarketState.CLOSED] == 1
    )
...
def test_main_execution_path(
    ...
    monkeypatch.setattr(
        run_service, "ask_password_if_needed", lambda *_args, **_kwargs: None
    )
    monkeypatch.setattr(
        run_service,
        "load_local_config",
        lambda *_args, **_kwargs: type(
            "_Cfg", (), {"rpc": {trades.Chain.GNOSIS.value: "http://rpc"}}
        )(),
    )
    monkeypatch.setattr(
        utils_module, "get_service_from_config", lambda *_args, **_kwargs: _Service()
    )
    operate_home = tmp_path / ".operate"
    operate_home.mkdir(parents=True)
    ...
    requests_mock.post(
        subgraph_url_a,
        [
            {"json": {"data": {"fpmmTrades": []}}},
            {"json": {"data": {"fpmmTrades": []}}},
        ],
    )
    requests_mock.post(subgraph_url_b, json={"data": {"user": {}}})
    requests_mock.post(
        "http://rpc", [{"json": {"result": "0xa"}}, {"json": {"result": "0xb"}}]
    )
