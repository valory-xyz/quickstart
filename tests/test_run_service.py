...
def get_service_config(config_path: str) -> dict:
    ...
    SERVICE_ALIASES = {"predict_trader": "traderpearl"}
...
def check_docker_status(logger: logging.Logger, config_path: str) -> bool:
    ...
    for attempt in range(max_retries):
        logger.info(
            f"Checking ABCI container status (attempt {attempt + 1}/{max_retries})"
        )
        try:
            ...
            abci_containers = [
                ...
                if c.name.endswith(abci_suffix)
                and c.name.startswith(container_base_name)
            ]
            ...
            if not abci_containers:
                logger.error(
                    f"No ABCI container found with base name {container_base_name} (attempt {attempt + 1}/{max_retries})"
                )
            ...
            logger.info(
                f"ABCI Container {abci_container.name} status: {abci_container.status}"
            )
            ...
            if abci_container.status == "exited":
                ...
                logger.error(
                    f"ABCI Container {abci_container.name} exited with code {exit_code}"
                )
                ...
                logger.error(
                    f"ABCI Container {abci_container.name} is restarting. Last logs:"
                )
                logs = abci_container.logs(tail=50).decode("utf-8")
                logger.error(f"ABCI Container logs:\n{logs}")
            ...
            if not running_abci:
                if attempt == max_retries - 1:
                    return False
                logger.info(
                    f"Waiting {retry_delay} seconds for ABCI container to start..."
                )
                time.sleep(retry_delay)
                continue
...
def check_service_health(logger: logging.Logger, config_path: str) -> tuple[bool, dict]:
    ...
    while time.time() - start_monitoring < 120:  # Run for 2 minutes
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(
                    f"Health check passed (response time: {metrics['response_time']:.2f}s)"
                )
    ...
    logger.info(
        f"Health check completed successfully - {metrics['successful_checks']} checks passed"
    )
    return True, metrics
...
def get_token_config():
    ...
    return {
        "mode": {
            ...
            "OLAS": {"address": "your_olas_address", "decimals": 18},
        },
        "optimism": {
            "USDC": {
    ...
    if match:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(
                    f"Successfully funded {required_amount} {token_symbol} to {wallet_address} on {chain} chain"
                )
                ...
                try:
                    ...
                    new_balance = token_contract.functions.balanceOf(
                        wallet_address
                    ).call()
                    logger.info(
                        f"New balance: {new_balance / (10 ** decimals)} {token_symbol}"
                    )
                except Exception as e:
                ...
                error_msg = (
                    f"Tenderly API request failed with status {response.status_code}"
                )
                if response.text:
        ...
        if match:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    token_name = (
                        "ETH" if chain.ledger_type == LedgerType.ETHEREUM else "xDAI"
                    )

                    logger.info(
                        f"Successfully funded {required_amount} {token_name} to {wallet_type} {wallet_address}"
                    )

                    new_balance = w3.eth.get_balance(wallet_address)
                    logger.info(
                        f"New balance: {w3.from_wei(new_balance, 'ether')} {token_name}"
                    )
                    return ""
                else:
                    raise Exception(
    ...
    try:
        ...
        for container in containers:
            ...
            if "Error during shutdown" in logs or "Failed to gracefully stop" in logs:
                logger.error(
                    f"Found shutdown errors in container {container.name} logs"
                )
                return False
...
def ensure_service_stopped(
    config_path: str, temp_dir: str, logger: logging.Logger
) -> bool:
    """
    ...
    try:
        ...
        try:
            ...
            raise RuntimeError(
                "Docker daemon not running or not accessible. Please start Docker first."
            ) from docker_err
        ...
        if remaining_containers:
            logger.info(
                "Some containers still running after stop_service, forcing stop..."
            )
...
def get_base_config(config_path: str = "") -> dict:
    ...
    base_config = {
        ...
        "BACKUP_WALLET": validate_backup_owner(
    ...
    base_prompts = {
        r"Please input your password \(or press enter\)\:": base_config[
            "TEST_PASSWORD"
        ],
        r"Please confirm your password\:": base_config["TEST_PASSWORD"],
        r"Enter local user account password \[hidden input\]\:": base_config[
            "TEST_PASSWORD"
        ],
        r"Enter your choice\s*\(\s*\d+\s*-\s*\d+\s*\)\s*:\s*$": base_config[
            "STAKING_CHOICE"
        ],
        r"Please input your backup owner \(leave empty to skip\)\:": base_config[
            "BACKUP_WALLET"
        ],
        r"Press enter to continue\.*\s*$": "\n",
        r"Please enter .*(?:\[hidden input\])?:\s*$": lambda output, logger: handle_env_var_prompt(
            output, logger, config_path
        ),
    }
...
def get_config_specific_settings(config_path: str) -> dict:
    ...
    if "optimus" in config_path.lower():
        ...
        prompts.update(
            {
                r"Enter a Mode RPC that supports eth_newFilter \[hidden input\]": test_config[
                    "MODE_RPC"
                ],
                r"Enter a Optimism RPC that supports eth_newFilter \[hidden input\]": test_config[
                    "OPTIMISM_RPC"
                ],
                r"Enter a Base RPC that supports eth_newFilter \[hidden input\]": test_config[
                    "BASE_RPC"
                ],
                r"\[(?:optimism|base|mode)\].*Please transfer at least.*(?:ETH|xDAI) to the Master (EOA|Safe) (0x[a-fA-F0-9]{40})": lambda output, logger: create_funding_handler(
                    get_chain_rpc(output, logger)
                )(
                    output, logger
                ),
                r"\[(?:optimism|base|mode)\].*Please transfer at least.*(?:USDC|OLAS) to the Master (?:EOA|Safe) (0x[a-fA-F0-9]{40})": lambda output, logger: create_token_funding_handler(
                    get_chain_rpc(output, logger)
                )(
                    output, logger
                ),
        ...
        prompts.update(
            {
                ...
                r"Please transfer at least.*(?:ETH|xDAI) to the Master (EOA|Safe) (0x[a-fA-F0-9]{40})": lambda output, logger: create_funding_handler(
                    test_config["BASE_RPC"]
                )(
                    output, logger
                ),
            }
        ...
        prompts.update(
            {
                ...
                r"Please transfer at least.*(?:USDC|OLAS) to the Master (?:EOA|Safe) (0x[a-fA-F0-9]{40})": lambda output, logger: create_token_funding_handler(
                    test_config["RPC_URL"]
                )(
                    output, logger
                ),
...
class BaseTestService:
    ...
    def setup_class(cls):
        ...
        if "agents.fun" in cls.config_path.lower():
            temp_config_path = os.path.join(
                cls.temp_dir.name, "configs", os.path.basename(cls.config_path)
            )
            try:
                ...
                cls.logger.info(
                    f"Modified agents.fun config in temp dir: {temp_config_path}"
                )
    ...
    def _setup_environment(cls):
        ...
        if venv_path:
            if os.name == "nt":  # Windows
                ...
                site_packages = list(Path(venv_path).glob("lib/python*/site-packages"))[
                    0
                ]

            pythonpath = cls.temp_env.get("PYTHONPATH", "")
            cls.temp_env["PYTHONPATH"] = (
                f"{site_packages}:{pythonpath}" if pythonpath else str(site_packages)
            )
    ...
    def teardown_class(cls):
        ...
        try:
            ...
            try:
                ...
                if containers:
                    cls.logger.warning(
                        "Found running containers after stop_service, forcing removal..."
                    )
                    for container in containers:
                        container.stop(timeout=30)
                        container.remove()
    ...
    def start_service(cls):
        ...
        try:
            cls.logger.info(
                f"Starting run_service.py test with config: {cls.config_path}"
            )
    ...
    def test_shutdown_logs(self):
        ...
        assert (
            check_shutdown_logs(self.logger, self.config_path) == True
        ), "Shutdown logs check failed"
...
class TestAgentService(TempDirMixin):
    ...
    logger = setup_logging(
        Path(f'test_run_service_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log')
    )
    ...
    @pytest.mark.parametrize(
        "setup", get_config_files(), indirect=True, ids=lambda x: Path(x).stem
    )
