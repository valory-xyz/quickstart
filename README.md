...
```
where `ai_agent_config.json` is the path to your AI agent configuration file. Check the `configs/` directory for available configurations of different AI agents.
...
- Once a staking program is selected, you can reset your preference by stopping your AI agent by running `./stop_service.sh <ai_agent_config.json>` and then running the command

  ``` bash
  ./reset_staking.sh  <ai_agent_config.json>
  ```

  Keep in mind that your AI agent must stay for `minStakingDuration` in a staking program (typically 3 days) before you can change to a new program.
...
```bash
./analyse_logs.sh <ai_agent_config.json> --agent=aea_0 --reset-db
```

For example, inspect the state transitions using this command:

```bash
./analyse_logs.sh <ai_agent_config.json> --agent=aea_0 --reset-db --fsm
```
...
If you entered any input when running an AI agent for the first time, and later want to change them, use the following script:

```bash
./reset_configs.sh <ai_agent_config.json>
```
...
```bash
./claim_staking_rewards.sh <ai_agent_config.json>
```

The accrued OLAS will be transferred to your AI agent Safe without having to unstake your AI agent.

### Backups

AI agent runners are recommended to create a [backup](#backup-and-recovery) of the relevant secret key material.
...
Then continue above with [Run the script](#run-the-ai-agent).
...
- one smart contract account corresponds to a [Safe](https://app.safe.global/) wallet with a single owner (the AI agent account).
...
1. Ensure that your AI agent is stopped by running `stop_service.sh`.
...
```bash
./stop_service.sh <ai_agent_config.json>
./terminate_on_chain_service.sh <ai_agent_config.json>
```

## RPC-related Error Messages

When updating the AI agent, you may need to re-run the script if you obtain any of the following error messages:
...
## Build deployments without executing the agent instance
...
```bash
./run_service.sh <ai_agent_config.json> --build-only
```
