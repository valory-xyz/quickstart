"""Migrate a legacy `.optimus/` operator state directory into the modern `.operate/` layout."""
...
@dataclass
class OptimusConfig:
    """User-facing optimus quickstart configuration parsed from the legacy `.optimus/` JSON."""
...
def parse_optimus_config() -> OptimusConfig:
    ...
    principal_chain = (
        Chain.OPTIMISM.value
        if "optimistic" in rpc_mapping
        else next(iter(rpc_mapping), Chain.OPTIMISM.value)
    )
...
def main(config_path: Path) -> None:
    """Run the optimus-to-operate migration, copying state and rewriting config files."""
